# Workaround for OpenStack bug in Juno and Kilo releases, where user is not able to remove allowed_address_pairs via GUI (Horizon) 
# nor CLI after adding them with command-line client. This bug cannot be fixed by updating OpenStack CLI tools, it requires OpenStack 
# platform upgrade, and such a thing can be impossible depending on your role in the system (e.g. if you are just a tenant). This 
# is a script to remedy the situation by utilizing OpenStack's RESTful API, which is the only known workaround.
#
